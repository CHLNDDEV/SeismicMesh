...
def mkt2t(t):
    ...
    if dim == 1:
        ...
        edges = np.vstack(
            (t[:, [1, 2, 3]], t[:, [2, 3, 0]], t[:, [3, 0, 1]], t[:, [0, 1, 2]])
        )
    else:
        raise NotImplementedError
...
def assert_t2t_t2n(t2t, t2n):
    ...
    assert (
        t2t[mt2t.compressed(), mt2n.compressed()] == mt.compressed()
    ).all(), "inconsistent t2t"
    assert (
        t2n[mt2t.compressed(), mt2n.compressed()] == mn.compressed()
    ).all(), "inconsistent t2n"
...
def circumcenter(p, t):
    ...
    for it in range(nt):
        ...
        s = np.linalg.solve(
            np.array([[-dp1[1], dp2[1]], [dp1[0], -dp2[0]]]), -mid1 + mid2
        )
